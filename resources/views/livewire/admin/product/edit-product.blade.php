...
@push('scripts')
    <script>
        Livewire.on('ImageChoosen', () => {
            //When the image is choosed i'll read it using js
            let input = document.getElementById('image'); //input file type
            let file = input.files[0]; //the first and only file form the input
            let reader = new FileReader(); //reader result is the image data.
            reader.onloadend = () => {
                window.livewire.emit('fileUploaded', reader.result); //firing fileUpload event
            }
